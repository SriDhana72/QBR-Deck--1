console.log("script.js loaded!");
        ...
        document.addEventListener('DOMContentLoaded', () => {
            ...
            // --- Tab Switching Logic ---
            const navItems = document.querySelectorAll('.top-nav-item'); // Changed selector to target new class
            const contentSections = document.querySelectorAll('.content-section');

            const showTab = (targetId) => {
                // Deactivate all navigation items and hide all content sections
                navItems.forEach(nav => nav.classList.remove('active'));
                contentSections.forEach(section => {
                    section.classList.add('hidden');
                    // Remove existing layout classes before applying new ones
                    section.classList.remove('full-screen-content', 'newsletter-style-content');
                });

                // Find the target navigation item and content section
                const targetNavItem = document.querySelector(`.top-nav-item[data-target="${targetId}"]`); // Changed selector
                ...
                if (targetSection) {
                    ...
                    if (targetId === 'accountManagerDashboardContent' || targetId === 'merchant360Content') {
                        targetSection.classList.add('full-screen-content');
                    } else if (targetId === 'qbrDeckContent') {
                        targetSection.classList.add('newsletter-style-content');
                    }

                    // Special handling for QBR Deck animation when its tab is activated
                    if (targetId === 'qbrDeckContent') {
                        const qbrWordByWordTargets = targetSection.querySelectorAll('.word-by-word');
                        qbrWordByWordTargets.forEach(target => {
                            // Ensure words are wrapped if they haven't been (e.g., first time viewing QBR tab)
                            if (!target.dataset.wordsWrapped) {
                                wrapWords(target);
                                target.dataset.wordsWrapped = 'true';
                            }
                            // Reset opacity of words to 0 before re-animating
                            target.querySelectorAll('span').forEach(span => span.style.opacity = '0');
                            // Trigger the word-by-word animation
                            animateWords(target);
                        });
                    }
                    // Initial rendering for dashboard charts/table if applicable (when dashboard tab is activated)
            ...
            navItems.forEach(item => {
                item.addEventListener('click', () => {
                    showTab(item.dataset.target); // Call the showTab function with the clicked tab's target
                });
            });
            ...
            document.addEventListener('click', (event) => {
                ...
                // Ensure the modal is currently visible and the click was outside the modal itself and not on the button
                if (!filterModal.classList.contains('hidden') && !filterModal.contains(event.target) && event.target !== filterButton && !filterButton.contains(event.target)) {
                ...
                // Ensure the modal is currently visible and the click was outside the modal itself and not on the button
                if (!sortModal.classList.contains('hidden') && !sortModal.contains(event.target) && event.target !== sortButton && !sortButton.contains(event.target)) {
            ...
            // --- QBR Deck Animation Functions (Copied from previous QBR code) ---
            ...
            const wrapWords = (element) => {
                ...
                initialTextSpan.innerHTML = wrappedText; // Ensure innerHTML of initialTextSpan is updated
            };
            ...
            // Setup Intersection Observer for elements that come into view by scrolling within the tab
                ...
                entries.forEach(entry => {
                    if (entry.isIntersecting) {
                        if (entry.target.classList.contains('word-by-word')) {
                            ...
                            if (!entry.target.dataset.wordsWrapped) {
                                wrapWords(entry.target);
                                entry.target.dataset.wordsWrapped = 'true';
                            }
                            // Animate only if it's within the currently active QBR tab content
                            const parentTabContent = entry.target.closest('#qbrDeckContent');
                            if (parentTabContent && !parentTabContent.classList.contains('hidden')) {
                                // Reset opacity before animating to ensure re-play on scroll
                                entry.target.querySelectorAll('span').forEach(span => span.style.opacity = '0');
                                animateWords(entry.target); // Corrected to use entry.target
                            }
                        }
                    }
                });
            ...
            // Observe the word-by-word elements within the QBR deck content area
            const qbrDeckElement = document.getElementById('qbrDeckContent');
            if (qbrDeckElement) {
                const qbrWordByWordTargets = qbrDeckElement.querySelectorAll('.word-by-word');
                qbrWordByWordTargets.forEach(target => {
            ...
            // Explicitly show the Account Manager's Dashboard on initial page load
            showTab('accountManagerDashboardContent');
        });